...
function montarp(requ) {
//	console.log('-----RPrequ:'+requ+'-----');
	var reqp = requ.toString().split(',');
	var rp = '';
...
//		console.log('where_user:'+queryUser);
...
console.log('get/midias');		
		var connection = mysql.createConnection(dbConfig.connection);
		connection.query('USE ' + dbConfig.database);

		var query = "select distinct corretor_type from ga_data where trim(corretor_type) <> ''";
...
//			if (rp.length>0) rp = rp.substring(0,rp.length-1);
//			whereFoundFilter += 'and CONCAT(SCE.ev," ",SCE.ea," ",SCE.aa) in ('+rp+') '
//		}
...
//		console.log('AAAAAAAAAY'+whereFoundFilter+'aaaaaaaaaY');
		var queryFoundFilter = 
			'SELECT '+
				'CONCAT(SCE.ev," ",SCE.ea," ",SCE.aa) estruturaVenda '+
			'FROM  '+
